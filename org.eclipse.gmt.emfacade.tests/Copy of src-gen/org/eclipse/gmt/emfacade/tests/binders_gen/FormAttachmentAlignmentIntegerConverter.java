package org.eclipse.gmt.emfacade.tests.binders_gen;

import org.eclipse.gmt.emfacade.builder.AbstractConverter;
import org.eclipse.gmt.emfacade.builder.IConverter;
import org.eclipse.gmt.emfacade.builder.IConverterContext;
import org.eclipse.gmt.emfacade.swt.FormAttachmentAlignment;

import org.eclipse.gmt.emfacade.swt.SwtPackage;

public class FormAttachmentAlignmentIntegerConverter extends AbstractConverter<FormAttachmentAlignment, Integer> implements IConverter<FormAttachmentAlignment, Integer> {

	@Override
	public Integer convert(FormAttachmentAlignment model, IConverterContext context) {
		
		int _value = model.getValue();
		return _value;
	}
}
